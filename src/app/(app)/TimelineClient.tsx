...
export default function TimelinePage({ initialPosts, initialHasMore }: TimelinePageProps) {
  ...
  useEffect(() => {
    // Skip first fetch if we have server-prefetched data for the "all" tab
    if (initialFetchDone.current && tab === "all") {
      initialFetchDone.current = false;
      lastFetchRef.current = Date.now();
      postsCache.current.set("all", initialPosts || []);
      return;
    }
    setPage(1);
    fetchPosts(1, true);
  }, [tab, fetchPosts, initialPosts]);
